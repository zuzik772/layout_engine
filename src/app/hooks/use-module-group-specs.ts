...
export function useModuleGroupSpecs(id: number) {
  ...
  const {
    ...
    initialData: () => {
      ...
      return queryClient.getQueryData<ModuleSpec[]>([queryKeys.moduleGroupSpecs, id]);
    },
  });
  ...
  const addModuleSpecMutation = useMutation({
    ...
    onMutate: async (newSpec: ModuleSpec) => {
      ...
      const previousModuleGroupSpecs = queryClient.getQueryData<ModuleSpec[]>([queryKeys.moduleGroupSpecs, id]);
      //optimistic update to the new spec
      queryClient.setQueryData<ModuleSpec[]>([queryKeys.moduleGroupSpecs, id], (old = []) => [...old, newSpec]);
      return { previousModuleGroupSpecs };
    },
    onError: (err, newSpec, context) => {
      queryClient.setQueryData<ModuleSpec[]>([queryKeys.moduleGroupSpecs, id], context?.previousModuleGroupSpecs);
  ...
  const deleteModuleSpecMutation = useMutation({
    ...
    onMutate: async (id: number) => {
      ...
      const previousModuleGroupSpecs = queryClient.getQueryData<ModuleSpec[]>([queryKeys.moduleGroupSpecs, id]);
      queryClient.setQueryData<ModuleSpec[]>([queryKeys.moduleGroupSpecs, id], (old = []) => old.filter((spec) => Number(spec.id) !== id));
      return { previousModuleGroupSpecs };
    },
  ...
  const updateModuleSpecMutation = useMutation({
    mutationFn: (isDisabled: boolean) => updateModuleGroupSpec(id, isDisabled),
    // onMutate: async (spec: ModuleSpec) => {
    //   await queryClient.cancelQueries({
    //     queryKey: [queryKeys.moduleGroupSpecs, id],
    //   });
    //   const previousModuleGroupSpecs = queryClient.getQueryData<ModuleSpec[]>([queryKeys.moduleGroupSpecs, id]);
    //   queryClient.setQueryData<ModuleSpec[]>([queryKeys.moduleGroupSpecs, id], (old = []) =>
    //     old.map((s) => (s.id === spec.id ? { ...s, ...spec } : s))
    //   );
    //   return { previousModuleGroupSpecs };
    // },
